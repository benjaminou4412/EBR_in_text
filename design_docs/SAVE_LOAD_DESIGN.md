...
- We'll add a "Menu" option here, at the bottom, having it lead to a submenu with choices to save the gamestate, load a gamestate, or "return to title"
- The advantage of this is that we guaranteed the gamestate can only ever be saved between Tests and other actions during a player turn, which restricts the amount of edge cases we have to take into account
    - For example, if we allowed saving in the middle of the test, we would have to track the global-ish variable last_test_target
...
def instantiate_card(card_data: CardData) -> Card:
    ...
    # Card.__post_init__ generates a random UUID-based ID (e.g., "biscuit-basket-a1b2")
    # We must replace it with the saved ID so all references resolve correctly:
    # - ranger_token_location
    # - attached_to_id / attached_card_ids
    # - listener source_card_id
    # - constant ability source_card_id
...
**Important**: `reconstruct()` must NOT call `enters_play()` or `enters_hand()` on cards. It should only:
1. Call `get_listeners()` on cards in play and hand and register returned listeners
2. Call `get_constant_abilities()` on cards in play and register them
...
Double-sided cards (like BiscuitDelivery/BiscuitBasket) have mutual backside references.

**Current behavior**: When you instantiate a double-sided card with default `fresh=True`, it automatically creates and links its backside:
```python
# In BiscuitDelivery.__init__:
if fresh:
    self.backside = BiscuitBasket(fresh=False)
    self.backside.backside = self
```

**Saving**: Only save the card that's currently face-up (in an area, deck, hand, etc.). Include:
- `card_class`: The class of the face-up side
- `backside_class`: The class of the other side (for instantiation)
- Mutable state (tokens, exhausted, etc.) - this belongs to the physical card, not a specific side

**Loading**: Use `fresh=False` to prevent auto-creation of backside, then link manually:
```python
def instantiate_double_sided_card(card_data: CardData) -> Card:
    # Instantiate both sides with fresh=False to prevent auto-linking
    front_class = get_card_class(card_data.card_class)
    back_class = get_card_class(card_data.backside_class)

    front = front_class(fresh=False)
    back = back_class(fresh=False)

    # Override IDs (backside gets a derived ID)
    front.id = card_data.id
    back.id = f"{card_data.id}-backside"

    # Link backsides manually
    front.backside = back
    back.backside = front

    # Apply mutable state to the face-up side only
    # (flipping transfers state to the new face-up side automatically)
    apply_mutable_state(front, card_data)

    return front
```

**Note**: Mutable state (tokens, exhausted, etc.) belongs to the physical card, not a specific side. When a card flips in gameplay, the new face-up side inherits the same tokens/state. We only save state for the currently face-up side.
...
2. **Challenge deck state**: The physical deck order matters. We'll save full challenge deck and challenge discard state.
